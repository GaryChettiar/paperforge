  ...
  return (
    ...
    >
      ...
      <div className="pb-6 mb-7 border-b-4" style={{ borderColor: '#243e36' }}>
        <h1 className="text-3xl font-bold mb-2" style={{ color: '#243e36', lineHeight: 1.1, letterSpacing: '-0.5px' }}>
          {data.personalInfo.fullName}
        </h1>
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-gray-700 text-sm">
          {data.personalInfo.email && (
            <div className="flex items-center gap-2 min-w-[180px]">
              <Mail className="w-4 h-4 min-w-4 min-h-4" style={{ color: '#42697a' }} />
              <span>{data.personalInfo.email}</span>
            </div>
          )}
          {data.personalInfo.phone && (
            <div className="flex items-center gap-2 min-w-[140px]">
              <Phone className="w-4 h-4 min-w-4 min-h-4" style={{ color: '#42697a' }} />
              <span>{data.personalInfo.phone}</span>
            </div>
          )}
          {data.personalInfo.location && (
            <div className="flex items-center gap-2 min-w-[120px]">
              <MapPin className="w-4 h-4 min-w-4 min-h-4" style={{ color: '#42697a' }} />
              <span>{data.personalInfo.location}</span>
            </div>
          )}
          {data.personalInfo.linkedin && (
            <div className="flex items-center gap-2 min-w-[130px]">
              <Linkedin className="w-4 h-4 min-w-4 min-h-4" style={{ color: '#42697a' }} />
              <span>{data.personalInfo.linkedin}</span>
            </div>
          )}
          {data.personalInfo.website && (
            <div className="flex items-center gap-2 min-w-[130px]">
              <Globe className="w-4 h-4 min-w-4 min-h-4" style={{ color: '#42697a' }} />
      ...
      {data.summary && (
        <div className="mb-7">
          <h2 className="text-xl font-semibold mb-3" style={{ color: '#243e36', letterSpacing: '-1px' }}>
          ...
          <div className="space-y-5">
            {data.experience.map((exp) => (
              <div key={exp.id} className="pl-4 border-l-2" style={{ borderColor: '#f1f7ed' }}>
                ...
                <div className="flex flex-wrap gap-6 text-base font-medium" style={{ color: '#243e36' }}>
          ...
          <div className="space-y-3">
            {data.education.map((edu) => (
              <div key={edu.id}>
                ...
                <div className="flex flex-wrap gap-6" style={{ color: '#243e36' }}>
      ...
      {data.skills.length > 0 && (
        <div className="mb-7">
          ...
          <div
            ...
            style={{ rowGap: '10px', columnGap: '12px', alignItems: 'center' }}
          >
