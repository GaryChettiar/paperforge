...
export const CreativeTemplate: React.FC<CreativeTemplateProps> = ({ data }) => {
  return (
    <div className="w-full max-w-4xl mx-auto bg-white text-sm leading-relaxed">
      <div className="flex">
        {/* Left Sidebar */}
        <div className="w-1/3 text-white p-6" style={{ background: `linear-gradient(180deg, #0d3b66 0%, #f4d35e 100%)` }}>
          <div className="mb-6">
        ...
        <div className="w-2/3 p-6">
          ...
          {data.summary && (
            <div className="mb-6">
              <h2 className="text-xl font-bold mb-3 pb-2" style={{ color: '#0d3b66', borderBottom: `2px solid #faf0ca` }}>
                About Me
              </h2>
              ...
              <div className="space-y-4">
                {data.experience.map((exp) => (
                  <div key={exp.id} className="relative pl-6">
                    <div className="absolute left-0 top-2 w-3 h-3 rounded-full" style={{ backgroundColor: '#0d3b66' }}></div>
                    <div className="absolute left-1.5 top-5 w-0.5 h-full" style={{ backgroundColor: '#faf0ca' }}></div>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {data.projects.map((project) => (
                  <div key={project.id} className="p-3 rounded-lg" style={{ backgroundColor: '#faf0ca' }}>
                    <h3 className="font-bold text-gray-900">{project.name}</h3>
                    {project.url && <p className="text-xs" style={{ color: '#0d3b66' }}>{project.url}</p>}
                    <p className="text-gray-700 text-xs mb-2">{project.description}</p>
                    <div className="flex flex-wrap gap-1">
                      {project.technologies.map((tech) => (
                        <span key={tech} className="px-2 py-1 rounded-full text-xs" style={{ backgroundColor: '#f4d35e', color: '#0d3b66' }}>
                          {tech}
                        </span>
                      ))}
                    </div>
